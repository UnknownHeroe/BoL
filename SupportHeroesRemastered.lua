...
1) New BoL Tools code 
2) Fixed Morgana casting
3) Tweaked Morgana shield
4) Add logic to Sona passive
5) Added small bits of logic to other champions that is not yet finished. Left it in there because it'll still work.
]]

local scriptVersion = 1.44
...
function _Bundle:__init()
	self:Print("Unless someone like you cares a whole awful lot, nothing is going to get better. IT'S NOT! - Lorax")
...
function _MovementBlock:SendPacket(p)
	if p.header == 0x10C and self.blockMovement then
		p:Block()
	end
end
